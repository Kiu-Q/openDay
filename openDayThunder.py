...
INTRO = [["Level 1: Score EXACTLY 30 points as soon as possible.",
         ...
         ["Level 3: SURVIVAL MODE vs COMPUTER.",
          ...
          "THUNDER power-up appears at 10+ marks!",
          "Press to start",
          "Top 5 Scores: "]]
...
class Game(Main):
    def __init__(self):
        ...
        try:
            ...
            self.compClev = 0.0
            self.cleverness_multiplier = 1.0 # New permanent multiplier
        except:
            self.prev = 0
            self.level = 1
            self.compClev = 0.0
    ...
    def level3(self):
        ...
        self.compScore = 0
        self.level = 3
        self.cleverness_multiplier = 1.0 # Reset multiplier for level start
        self.player = Shooter()
    ...
    def run(self, level):
        ...
        # Thunder spawn logic for Level 3
        if level == 3 and self.score >= 10: # Changed score threshold to 10
            # Check if there's no Thunder currently on screen
            hasThunder = any(isinstance(t, Thunder) for t in self.targets)
            if not hasThunder and random.randint(0, 100) < 3:  # Decreased chance to 3%
        ...
        if level == 1:
            ...
            # self.compClev is the effective cleverness calculated in Computer.updateAi
        ...
        if level == 3:
            ...
            for target in self.targets[:]:  # Use slice to avoid modification issues
        ...
        if results.multi_hand_landmarks or level != 3:
            for target in self.targets[:]:  # Use slice to avoid modification issues
                if target.collide(self.player.pos):
                    ...
                    if isinstance(target, Thunder) and level == 3:
                        BEEPH.play()
                        
                        # Permanent cleverness reduction: Halve the multiplier
                        self.cleverness_multiplier /= 2.0
                        self.cleverness_multiplier = max(0.05, self.cleverness_multiplier) # Prevent it from becoming too small

                        # Kill all monsters on screen and grant score
                        monstersToRemove = [t for t in self.targets if isinstance(t, Monster)]
                        for monster in monstersToRemove:
                            self.score += 1
                            self.screen.blit(EXPLODE, (monster.pos[0]-100, monster.pos[1]-100))
                            if monster in self.targets:
                                self.targets.remove(monster)
                        
                        # Remove the thunder itself
                        if target in self.targets:
                            self.targets.remove(target)
                    
                    elif isinstance(target, Ballon) or isinstance(target, Monster):
...
class Computer(Object):
    ...
    def updateAi(self, targets, timeElap, plrPos):
        x = timeElap
        baseClev = 0.2 + 0.8 * (x**2 / (x**2 + CLEVER**2))
        
        # Apply the permanent cleverness multiplier
        currentClev = baseClev * self.game.cleverness_multiplier
        
        # Store the effective cleverness for display
